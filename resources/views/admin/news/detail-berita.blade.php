  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header card-header-primary" style="margin-top: -2.5rem">
        <h2 class="card-title h2 fw-bolder text-center text-capitalize">{{ $feed->judul }}</h2>
      </div>
      <div class="card-body">
        ...
        {!! html_entity_decode ($feed->deskripsi) !!}
      </div>
      <div class="card-footer">
        {{-- widget goes here --}}
        <p class="card-category">Dibuat oleh {{ $feed->user->name }} pada {{ \Carbon\Carbon::parse($feed->tanggal)->format('d F Y')}}</p>
      </div>
    </div>
  </div>
  <!-- end col-md-12 -->
